...
using MythHunter.Core.DI;
using Cysharp.Threading.Tasks; // Додано імпорт UniTask
...
{{
    ...
    {{
        ...
        public async UniTask InitializeAsync() // Змінено на асинхронний з UniTask
        {{
            ...
            await UniTask.CompletedTask; // Для асинхронної сигнатури
        }}
...
{{
    ...
    {{
        ...
        private async void Awake() // Змінено на async void для підтримки UniTask
        {{
            if (view == null)
            ...
            await _presenter.InitializeAsync(); // Використання асинхронного методу
        }}
